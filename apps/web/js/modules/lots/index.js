...
export function createLotsModule({ state, supabase, select, escapeHtml, totalHa, latest, vigor, openModal, openOrder, loadData, render }) {
  ...
  function openEditor(lot = {}) {
    ...
    form.onsubmit = async event => {
      ...
      try {
        ...
        const row={company_id:state.companyId,name:String(f.get('name')).trim(),crop:String(f.get('crop')).trim()||null,variety:String(f.get('variety')).trim()||null,hectares,area_ha:hectares,status:f.get('status'),sowing_date:f.get('sowing_date')||null,last_cut:f.get('last_cut')||null,last_irrigation:f.get('last_irrigation')||null,soil_moisture:f.get('soil_moisture')===''?null:num(f.get('soil_moisture')),next_task:String(f.get('next_task')).trim()||null,notes:String(f.get('notes')).trim()||null,updated_at:new Date().toISOString()};
  ...
  function openDetails(lot) {
    ...
    openModal(`<div class="lot-command"><div class="lot-command-head"><div><p class="eyebrow">CENTRO OPERATIVO DEL LOTE · 3.0.0</p><h2>${escapeHtml(lot.name)}</h2><p class="muted">${escapeHtml(lot.crop||'Sin cultivo')} · ${escapeHtml(lot.variety||'Sin variedad')} · ${ha(hectares)} ha</p></div><div class="actions"><span class="lot-health ${health.className}">${health.icon} ${health.label}</span><button class="secondary editLotFromDetail">Editar</button></div></div>
