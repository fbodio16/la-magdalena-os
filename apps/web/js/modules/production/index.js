...
export function createProductionModule({ state, supabase, select, escapeHtml, openModal, loadData, render }) {
  ...
  function renderPage() {
    ...
    <div class="panel"><div class="panel-title"><h3>Historial económico y productivo</h3><span class="pill">${cuts.length} registros</span></div>${cuts.length?`<div class="production-table-wrap"><table class="table"><thead><tr><th>Fecha</th><th>Lote</th><th>Corte</th><th>Ha</th><th>Rollos</th><th>Rinde</th><th>Calidad</th><th>Destino</th><th>Ingreso</th><th>Costo</th><th>Margen</th></tr></thead><tbody>${cuts.map(c=>{const lot=state.lots.find(l=>l.id===c.lot_id),margin=cutRevenue(c)-cutCost(c);return `<tr><td>${escapeHtml(c.cut_date)}</td><td><b>${escapeHtml(lot?.name||'Sin lote')}</b></td><td>${c.cut_number||'—'}</td><td>${number(c.hectares,1)}</td><td>${c.rolls}</td><td>${c.hectares?number(c.rolls/c.hectares,1):'—'}</td><td><span class="pill">${escapeHtml(c.quality)}</span></td><td>${escapeHtml(c.destination)}</td><td>${money(cutRevenue(c))}</td><td>${money(cutCost(c))}</td><td class="${margin<0?'negative':'positive'}">${money(margin)}</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty"><b>No hay cortes registrados.</b><p>Registrá el primer corte para comenzar el seguimiento.</p></div>'}</div>`;
  }
  ...
  function bind() {
    document.querySelectorAll('.newCut').forEach(b=>b.onclick=()=>openCutEditor());document.querySelectorAll('.exportProduction').forEach(b=>b.onclick=exportCsv);
