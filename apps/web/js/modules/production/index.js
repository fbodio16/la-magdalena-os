...
export function createProductionModule({ state, supabase, select, escapeHtml, openModal, loadData, render }) {
  ...
  async function saveStockMovement(event) {
    ...
    try { if(!cut) throw new Error('No se encontró la partida.'); const quantity=Number(form.get('quantity')||0), type=form.get('type'); if(!(quantity>0)) throw new Error('La cantidad debe ser mayor que cero.'); if(type==='Salida'&&quantity>stockRollsOf(cut)) throw new Error('La salida supera el stock disponible.'); const movement={id:`MOV-${Date.now()}`,type,quantity,date:form.get('date'),reason:form.get('reason'),customer:form.get('customer')||'',storage_location:form.get('storage_location')||cut.storage_location||'Sin asignar',notes:form.get('notes')||'',created_at:new Date().toISOString()}; const metadata={...parseCutMetadata(cut),storage_location:movement.storage_location,stock_movements:[...(cut.stock_movements||[]),movement]}; if(type==='Salida'&&movement.reason==='Venta') metadata.sold_rolls=Number(metadata.sold_rolls||0)+quantity; const {error}=await supabase.from('alfalfa_cuts').update({notes:`LMOS_CUT:${JSON.stringify(metadata)}`}).eq('id',cut.id); if(error) throw error; select('#modalRoot').innerHTML=''; await loadData(); render(); } catch(error){msg.textContent=error.message;msg.classList.remove('hidden');}
  }
